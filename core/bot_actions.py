        ...
        if message and isinstance(message, list) and len(message) > 0:
            ...
            for reply_segment in reply_segments:
                ...
                async with aiohttp.ClientSession() as client:
                    response = await client.post(endpoint, json=payload, headers=headers, proxy=SEND_PROXY)
            ...
            if text_segments:
                ...
                async with aiohttp.ClientSession() as client:
                    response = await client.post(endpoint, data=str(payload).encode('utf-8'), headers=headers, proxy=SEND_PROXY)
